      ...
      <aside className="sidebar w-56 flex-shrink-0 hidden lg:flex flex-col">
        <div className="p-4 border-b border-green-500/10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="font-bold text-green-400 neon-text text-sm leading-tight">Rip Portal</div>
              <div className="text-[10px] text-zinc-500 tracking-wider">PACK EV CALCULATOR</div>
            </div>
          </div>
        </div>

        <nav className="flex-1 py-4 px-2.5 space-y-0.5 overflow-y-auto">
          <div className="text-[10px] uppercase tracking-widest text-zinc-600 px-3 mb-2">Tools</div>
          {[
            ...
            >
              ...
              {"pro" in item && item.pro && (
                <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30 uppercase tracking-wide">Pro</span>
              )}
              {"soon" in item && item.soon && (
                <span className="ml-auto text-[9px] text-zinc-600 uppercase tracking-wide">Soon</span>
              )}
            </button>
          ))}
        </nav>
          ...
          <div className="px-4 md:px-6 py-3 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="lg:hidden flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 portal-glow flex items-center justify-center text-sm font-bold text-black">🌀</div>
                <span className="font-bold text-green-400 neon-text text-sm">Rip Portal</span>
              </div>
            ...
            <span className="hidden sm:inline-flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full bg-green-500/10 border border-green-500/30 text-green-300">⚡ Free Access</span>
          </div>
        </header>

        <main className="flex-1 p-4 md:p-6 overflow-y-auto">
          <div className="mb-5">
            <div className="text-[11px] uppercase tracking-[0.2em] text-green-400/70 mb-1.5">Calculate. Rip. Repeat.</div>
            <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">
              Pack EV. <span className="text-green-400 neon-text">Perfected.</span>
            </h1>
            <p className="text-sm text-zinc-500 mt-1 max-w-lg">Expected value for Pokémon, Sports & One Piece — know before you rip.</p>
          </div>
            ...
            <div className="mb-5 panel rounded-2xl p-4 border border-emerald-500/35 relative overflow-hidden">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5">
                <div className="shrink-0">
                  <div className="text-[10px] uppercase tracking-widest text-emerald-400/90 font-semibold mb-0.5">Best EV this week</div>
                ...
                <div className="flex-1 flex flex-wrap gap-2">
                  {[
                    ...
                    <div key={p.name} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-black/50 border border-emerald-500/25">
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">VALUE</span>
                      <div>
          ...
          <div className="flex gap-2 mb-4 lg:hidden">
            <button onClick={() => setView("calculator")} className={`flex-1 py-2 rounded-xl text-sm font-medium border ${view === "calculator" ? "bg-green-500/15 border-green-400/60 text-green-300" : "border-zinc-800 text-zinc-400"}`}>⚡ EV</button>
            <button onClick={() => setView("cards")} className={`flex-1 py-2 rounded-xl text-sm font-medium border ${view === "cards" ? "bg-cyan-500/15 border-cyan-400/60 text-cyan-300" : "border-zinc-800 text-zinc-400"}`}>🃏 Cards</button>
            <button onClick={() => setView("insider")} className={`flex-1 py-2 rounded-xl text-sm font-medium border ${view === "insider" ? "bg-amber-500/15 border-amber-400/60 text-amber-300" : "border-zinc-800 text-zinc-400"}`}>🔐 VIP</button>
          </div>
          ...
          {view === "cards" ? (
              ...
              {selectedCard && (
                <div className="panel rounded-2xl p-5 border border-cyan-500/40 portal-glow">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="w-14 h-20 rounded-xl bg-gradient-to-br from-cyan-700 to-purple-600 flex items-center justify-center text-3xl shrink-0 border border-white/10">{selectedCard.emoji || "🃏"}</div>
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-zinc-500 mt-0.5">{selectedCard.number && <span className="mr-2">#{selectedCard.number}</span>}{selectedCard.rarity}</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    <div className="rounded-xl bg-zinc-900/80 border border-zinc-800 p-3 text-center">
                      ...
                      <div className="text-[10px] text-zinc-500 mt-1">${selectedCard.rawLow}–${selectedCard.rawHigh}</div>
                    </div>
                    <div className="rounded-xl bg-zinc-900/80 border border-zinc-800 p-3 text-center">
                      <div className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1">PSA 9</div>
                      <div className="text-xl font-mono font-bold text-cyan-300">{selectedCard.psa9 ? `$${selectedCard.psa9.toLocaleString()}` : "—"}</div>
                    </div>
                    <div className="rounded-xl bg-zinc-900/80 border border-amber-500/30 p-3 text-center">
                      ...
                      <div className="text-xl font-mono font-bold text-amber-300">{selectedCard.psa10 ? `$${selectedCard.psa10.toLocaleString()}` : "—"}</div>
                    </div>
                  </div>
            ...
            <div className="space-y-5 max-w-4xl">
              <div className="panel rounded-2xl p-5 border border-amber-500/30 relative overflow-hidden">
                ...
                <div className="relative">
                  ...
                  <p className="text-sm text-zinc-400 max-w-xl mb-1">First 50 only. Best EV ranks, avoid list, early set data, deeper odds.</p>
                  <p className="text-xs text-amber-300/80 mb-4">$0 for 3 months → then $4.99/mo. Free calculator stays free forever.</p>
                  {claimDone ? (
                    <div className="rounded-xl border border-green-500/40 bg-green-500/10 p-4">
                      ...
                      <div className="text-xs text-zinc-400">Spot locked for <span className="text-zinc-200">{claimEmail}</span>.</div>
                    </div>
                  ) : (
                    <form onSubmit={handleClaim} className="space-y-3 max-w-md">
                      <div>
                        ...
                        <input type="text" value={claimName} onChange={(e) => setClaimName(e.target.value)} placeholder="Collector name" className="w-full bg-black/60 border border-zinc-700 rounded-xl px-3 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-amber-400/60" />
                      </div>
                      <div>
              ...
              <div className="panel rounded-2xl p-5">
                <h3 className="text-sm font-semibold text-white mb-4">Best EV Packs This Week</h3>
                <div className="space-y-2">
                  {[
                    { rank: 1, name: "Journey Together Pack", price: 3.75, ev: 4.1, roi: 9, note: "Strong IR/SIR floor" },
                    { rank: 2, name: "Surging Sparks Pack", price: 4.25, ev: 4.6, roi: 8, note: "Consistent modern value" },
                    { rank: 3, name: "OP-16 Booster Box", price: 95, ev: 102, roi: 7, note: "Box math cleaner" },
                    { rank: 4, name: "2026 Topps Series 1 Blaster", price: 25, ev: 26.5, roi: 6, note: "Cheap entry" },
                    ...
                    <div key={row.rank} className="flex items-center gap-3 p-3 rounded-xl bg-zinc-900/60 border border-zinc-800/80">
                      <div className="w-7 h-7 rounded-lg bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-xs font-bold text-emerald-300">{row.rank}</div>
                      <div className="flex-1 min-w-0">
              ...
              <div className="panel rounded-2xl p-5">
                <h3 className="text-sm font-semibold text-white mb-4">Avoid These (Overpriced)</h3>
                <div className="space-y-2">
                  {[
                    { name: "Ascended Heroes Pack", price: 14, ev: 8.3, roi: -41, note: "Chase tax — fun, not EV" },
                    { name: "Prismatic Evolutions Pack", price: 15, ev: 7.2, roi: -52, note: "Still inflated" },
                    { name: "Chrome Update Hobby", price: 950, ev: 720, roi: -24, note: "Need huge hits" },
                  ].map((row) => (
                    <div key={row.name} className="flex items-center gap-3 p-3 rounded-xl bg-red-500/5 border border-red-500/20">
                      <div className="w-7 h-7 rounded-lg bg-red-500/15 border border-red-500/30 flex items-center justify-center text-xs text-red-300">✕</div>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm text-zinc-100 font-medium truncate">{row.name}</div>
                        <div className="text-[11px] text-zinc-500 truncate">{row.note}</div>
                      </div>
            ...
            <div className="flex flex-col gap-5">
              <div>
                <h2 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest mb-2">Select Pack / Box</h2>
                    ...
                    return (
                      <button key={p.id} onClick={() => handleSelectProduct(p.id)} className={`snap-start flex-shrink-0 w-[140px] lg:w-auto text-left rounded-xl border overflow-hidden transition-all card-hover ${isActive ? "border-green-400/70 portal-glow ring-1 ring-green-400/30" : "border-zinc-800/80 hover:border-green-500/30"}`}>
                        ...
                        <div className="p-2.5 bg-zinc-900/90">
                          <div className="font-semibold text-zinc-100 text-xs leading-tight line-clamp-2">{p.name}</div>
                          <div className="text-[10px] text-zinc-500 mt-0.5 truncate">{p.format}</div>
                          <div className="text-[11px] text-green-400/80 mt-1 font-mono">~${p.defaultPrice.toFixed(2)}</div>
                        </div>
                      </button>
                    );
                  })}
                  ...
                  <div className="panel rounded-2xl p-4 md:p-5 portal-border">
                    <div className="flex gap-3 md:gap-4 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-zinc-400 mt-0.5">{effectiveProduct.format}</p>
                        <div className="flex flex-wrap items-end gap-3 mt-3">
                          <div>
                            <label className="block text-[10px] text-zinc-500 mb-1 uppercase tracking-wider">Your Price ($)</label>
                            <input type="number" inputMode="decimal" step="0.01" min="0" value={customPrice !== "" ? customPrice : price} onChange={(e) => setCustomPrice(e.target.value)} className="w-28 bg-black/70 border border-zinc-700 rounded-lg px-3 py-2.5 text-right text-green-300 font-mono text-sm focus:outline-none focus:border-green-400" />
                          </div>
                          <div>
                            <label className="block text-[10px] text-zinc-500 mb-1 uppercase tracking-wider">Qty</label>
                            <div className="flex items-center gap-1">
                              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-9 h-10 rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-300 text-lg">−</button>
                              <span className="w-9 text-center font-mono text-sm">{quantity}</span>
                              <button onClick={() => setQuantity(quantity + 1)} className="w-9 h-10 rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-300 text-lg">+</button>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {result && (
                      <div className={`rounded-xl p-4 mb-4 border ${totalProfit >= 0 ? "bg-green-500/10 border-green-400/40 portal-glow" : "bg-red-500/10 border-red-400/30"}`}>
                        <div className="flex flex-wrap items-center justify-between gap-4">
                          <div>
                            <div className="text-[10px] uppercase tracking-widest text-zinc-400 mb-1">Expected Value</div>
                            <div className="text-3xl md:text-4xl font-bold font-mono text-white tracking-tight">${result.totalEV.toFixed(2)}</div>
                            <div className="text-sm text-zinc-400 mt-1">per unit{quantity > 1 && <span className="text-zinc-500"> · ${totalEVScaled.toFixed(2)} total</span>}</div>
                          </div>
                          <div className="text-right">
                            <div className="text-[10px] uppercase tracking-widest text-zinc-400 mb-1">vs Price</div>
                            <div className={`text-xl md:text-2xl font-bold font-mono ${totalProfit >= 0 ? "text-green-400" : "text-red-400"}`}>{totalProfit >= 0 ? "+" : ""}${totalProfit.toFixed(2)}</div>
                            <div className={`text-sm font-mono ${roi >= 0 ? "text-green-400/80" : "text-red-400/80"}`}>{roi >= 0 ? "+" : ""}{roi.toFixed(1)}% ROI</div>
                          </div>
                          <div className="flex flex-col items-center">
                            <svg width="64" height="64" className="-rotate-90">
                              <circle cx="32" cy="32" r="26" fill="none" stroke="#1a1a24" strokeWidth="5" />
                              <circle cx="32" cy="32" r="26" fill="none" stroke={roi >= 0 ? "#39ff14" : "#f87171"} strokeWidth="5" strokeLinecap="round" strokeDasharray={2 * Math.PI * 26} strokeDashoffset={2 * Math.PI * 26 - (roiVisual / 100) * 2 * Math.PI * 26} className="transition-all duration-500" />
                            </svg>
                            <span className={`text-xs font-bold font-mono -mt-9 ${roi >= 0 ? "text-green-400" : "text-red-400"}`}>{roi >= 0 ? "+" : ""}{Math.round(roi)}%</span>
                            <span className="text-[9px] text-zinc-500 mt-4">ROI</span>
                          </div>
                        </div>
                        <div className="mt-3 text-sm">{totalProfit >= 0 ? <span className="text-green-300">▲ Positive EV — math favors opening on average</span> : <span className="text-red-300/90">▼ Negative EV — paying a premium for the chase / fun</span>}</div>
                      </div>
                    )}

                    <div>
                      <h3 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest mb-2">Value Breakdown</h3>
                      <div className="space-y-0.5">
                        {effectiveProduct.slots.map((slot, i) => {
                          const contribution = slot.oddsNum * slot.avgValue;
                          const pct = result && result.totalEV > 0 ? (contribution / result.totalEV) * 100 : 0;
                          return (
                            <div key={i} className="flex items-center justify-between text-sm py-2 border-b border-zinc-800/50 last:border-0">
                              ...
                              <div className="text-right">
                                <div className="font-mono text-zinc-300 text-sm">${contribution.toFixed(2)}</div>
                    ...
                    {effectiveProduct.notes && <p className="mt-4 text-[11px] text-zinc-500 leading-relaxed border-t border-zinc-800/50 pt-3">{effectiveProduct.notes}</p>}
                  </div>
                </div>

                <div className="lg:col-span-4 space-y-4">
                  <div className="panel rounded-2xl p-4">
                    <h3 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest mb-3">Market Pulse</h3>
                  ...
                  <div className="panel rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest">Recent Rips</h3>
                      <span className="text-[9px] text-zinc-600">Sample</span>
                    </div>
                    <div className="space-y-2.5">
                      {RECENT_RIPS.map((r, i) => (
                        <div key={i} className="flex items-center justify-between text-sm border-b border-zinc-800/40 pb-2 last:border-0 last:pb-0">
                          <div className="min-w-0">
                            <div className="text-zinc-200 truncate text-[12px]">{r.set}</div>
                            <div className="text-[10px] text-zinc-500">{r.packs} pack{r.packs > 1 ? "s" : ""}</div>
                          </div>
                          <div className="text-right shrink-0">
                            <div className="font-mono text-[12px] text-zinc-300">${r.ev.toFixed(2)}</div>
                            <div className={`text-[10px] font-mono ${r.roi >= 0 ? "text-green-400" : "text-red-400/80"}`}>{r.roi >= 0 ? "+" : ""}{r.roi}%</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {topSlot && (
                    <div className="panel rounded-2xl p-4">
                      <h3 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest mb-3">Top EV Contributor</h3>
                      ...
                      <div className="text-lg font-mono text-green-400">${(topSlot.oddsNum * topSlot.avgValue).toFixed(2)}</div>
                    </div>
                  )}
                  <p className="text-[10px] text-zinc-600 leading-relaxed px-1">Values are approximate. Decision aid only — not financial advice.</p>
                </div>
              </div>
            </div>
          ) : (
